      ...
      <div class="col-md-12">
        <div class="card mb-4">
          <div class="card-header pb-0">
            <h5>Affiliate Customer</h5>
            <p>Manage Affiliate Customers.</p>
            <div class="d-flex">
            </div>
          </div>
          <div class="card-body">
            <data-table
              ref="datatable"
              :id="'mytable'"
              :url="route('affilate-customer.getData', {id:customer_id})"
              :columns="columns"
...
props:{
  ...
  mounted() {
    ...
    $("#mytable tbody").on("click", "tr .action_affiliate_customer", (evt) => {
      const id = $(evt.target).data("item-id");
      this.$inertia.visit(route("affilate-customer.view", id));
    });
  ...
  methods: {
    ...
    updateStatus(id, status) {
      ...
      this.form.put(route("customer.change.status"), {
        preserveScroll: true,
        onSuccess: () => {
          this.form.reset();
          this.reloadTable();
