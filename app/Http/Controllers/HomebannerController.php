...
class HomebannerController extends Controller
{
    ...
    {
        $users = Homebanner::with('media')->get();
        // dd($users);
    ...
    {
        return Inertia::render('OtherCMS/Homebanner/CreateUpdate');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                $homebanner->addMedia($request->file('image'))->toMediaCollection('Home_Banner_Image');
                $homebanner->save();
            }
    ...
    public function updateStatus(Request $request)
    {
        // dd($request->all());
        try {
            $user = Homebanner::find($request->id);
            if ($request->status == 0) {
                $user->status = 1;
            } else {
                $user->status = 0;
    ...
    {
         $homebanner = Homebanner::with('media')->find($id);
        // dd($homebanner);
        return Inertia::render('OtherCMS/Homebanner/CreateUpdate', ['homebanner' =>  $homebanner]);
    }
    ...
    public function destroy(Request $request)
    {
        // dd($request->all());
        try {
            Homebanner::destroy([$request->ids]);
            return redirect()->route('homebanner.index');
        } catch (Exception $ex) {
            Log::error($ex);
            return abort(500);
